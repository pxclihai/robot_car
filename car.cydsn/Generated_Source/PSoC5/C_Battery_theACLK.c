...
/*******************************************************************************
* Function Name: C_Battery_theACLK_Start
********************************************************************************
*
* Summary:
...
void C_Battery_theACLK_Start(void) 
{
    ...
    C_Battery_theACLK_CLKEN |= C_Battery_theACLK_CLKEN_MASK;
	C_Battery_theACLK_CLKSTBY |= C_Battery_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: C_Battery_theACLK_Stop
********************************************************************************
*
* Summary:
...
void C_Battery_theACLK_Stop(void) 
{
    ...
    C_Battery_theACLK_CLKEN &= (uint8)(~C_Battery_theACLK_CLKEN_MASK);
	C_Battery_theACLK_CLKSTBY &= (uint8)(~C_Battery_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: C_Battery_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((C_Battery_theACLK_CLKEN & C_Battery_theACLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = C_Battery_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = C_Battery_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* C_Battery_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(C_Battery_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(C_Battery_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: C_Battery_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: C_Battery_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = C_Battery_theACLK_GetSourceRegister();
    uint16 oldDivider = C_Battery_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = C_Battery_theACLK_CLKEN & C_Battery_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(C_Battery_theACLK_DIV_PTR, clkDivider);
                C_Battery_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                C_Battery_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(C_Battery_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((C_Battery_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(C_Battery_theACLK_DIV_PTR, clkDivider);
				C_Battery_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: C_Battery_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(C_Battery_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: C_Battery_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    C_Battery_theACLK_MOD_SRC |= modeBitMask & (uint8)C_Battery_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: C_Battery_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    C_Battery_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(C_Battery_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: C_Battery_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return C_Battery_theACLK_MOD_SRC & (uint8)(C_Battery_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: C_Battery_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = C_Battery_theACLK_GetDividerRegister();
    uint8 oldSrc = C_Battery_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        C_Battery_theACLK_MOD_SRC |= CYCLK_SSS;
        C_Battery_theACLK_MOD_SRC =
            (C_Battery_theACLK_MOD_SRC & (uint8)(~C_Battery_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: C_Battery_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return C_Battery_theACLK_MOD_SRC & C_Battery_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: C_Battery_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    C_Battery_theACLK_PHASE = clkPhase & C_Battery_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: C_Battery_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return C_Battery_theACLK_PHASE & C_Battery_theACLK_PHASE_MASK;
}
